...
```
## Codebase Patterns
- Example: Prefer has_secure_token or UUIDs over incremental IDs for externally exposed identifiers.
- Example: Always make migrations idempotent by using IF NOT EXISTS, IF EXISTS, and reversible blocks.
- Example: Always use string type for enums instead of integers.
```
...
For any story that changes UI, you MUST verify it works in the browser. Use the system test functionality for that which uses a headless browser to check the UI.
